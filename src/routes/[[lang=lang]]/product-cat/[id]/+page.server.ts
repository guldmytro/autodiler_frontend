...
export const load: PageServerLoad = async ({ locals: { locale, LL }, fetch, params, url, cookies }) => {
    ...
    let fetchUrl = `${apiUrl}products/?envelope=true&omit=same_category_products,params,description,country,recommended_products&category=${term.id}&limit=${PUBLIC_PRODUCTS_PER_PAGE}`;
