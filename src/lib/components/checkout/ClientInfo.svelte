<script>
    ...
    let startedFillingUserInfo = false;
    let startedFillingShippingInfo = false;
    let startedFillingPaymentInfo = false;
    ...
    $: items = $page.data.cart.items.map(item => {
        return {
    ...
    function handleStartsPaymentInfo() {
        if (startedFillingPaymentInfo) return;
        startedFillingPaymentInfo = true;
    ...
    function handleStartsShippingInfo() {
        if (startedFillingShippingInfo) return;
        startedFillingShippingInfo = true;
        window.dataLayer = window.dataLayer || [];
        window.dataLayer.push({
    ...
    function handleStartFillingUserInfo() {
        if (startedFillingUserInfo) return;
        startedFillingUserInfo = true;
        window.dataLayer = window.dataLayer || [];
        window.dataLayer.push({
    ...
    <fieldset class="checkout-form__fieldset fieldset delivery">
        ...
        <div class="radio-box">
            <label class="label-radio">
                <input type="radio" class="radio" name="delivery" value="nd"
                    on:change={() => handleStartsShippingInfo()}>
    ...
    <fieldset class="checkout-form__fieldset fieldset">
        ...
        <div class="radio-box">
            <label class="label-radio" class:disabled={cart.total < 300}>
                <input type="radio" class="radio" name="payment_method" value="od" disabled={cart.total < 300}
                    on:change={() => handleStartsPaymentInfo()}>
                <span class="pseudo-radio"></span>
