<script>
    ...
    async function setNovaCity(city, DeliveryCity) {
        $novaDelivery.cityHref = DeliveryCity;
        $novaDelivery.novaCity = city;
        $novaDelivery.novaOffice = '';
        openDropDown = false;
        showNovaCityDropDown = false;
        searchQueryCity = '';
        cities = [];
...
<div class="shipping-select">
    <div class="dropdown">
        <label class="label-input">
            ...
            {#if form?.errors?.city === 'required'}
                <p class="errors">{$LL.checkoutErrors.city()}</p>
            {/if}
            <div class="dropdown-base">
                <input type="text" name="city" bind:value={$novaDelivery.novaCity} class="input dropdown__input" readonly placeholder={$LL.checkout.cityPlaceholder()} on:click={() => openDropDown = !openDropDown}>
